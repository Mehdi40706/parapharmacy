<template>
  <Teleport to="body">
    ...
    >
      <div
        v-if="product"
        class="fixed inset-0 z-[60] bg-ink/50 backdrop-blur-sm flex items-center justify-center p-4"
      ...
      >
        ...
        >
          <div
            class="bg-white rounded-2xl w-full relative overflow-hidden flex flex-col"
            :style="cardMaxWidth ? { maxWidth: cardMaxWidth } : { maxWidth: '640px' }"
          >
            <button
              @click="$emit('close')"
              class="absolute top-4 right-4 z-10 w-9 h-9 rounded-full bg-white/90 flex items-center justify-center hover:bg-mist transition-colors"
              aria-label="Fermer"
            >
            ...
            <!-- Image -->
            <div class="bg-mist overflow-hidden max-h-[50vh]">
              <img
                v-if="product.imageUrl"
                :src="product.imageUrl"
                :alt="product.name"
                class="w-full h-full max-h-[50vh] object-contain mx-auto"
                @load="handleImageLoad"
              />
              <div v-else class="w-full aspect-square flex items-center justify-center text-ink/20 min-h-[300px]">
            ...
            <!-- Détails -->
            <div class="p-6 flex flex-col overflow-y-auto max-h-[40vh]">
              <span v-if="product.category?.name" class="badge-pill bg-mist text-sage-dark self-start mb-3">
              ...
              <div class="flex items-center gap-3 mb-4">
                ...
                <span v-else-if="product.stock !== undefined" class="badge-pill bg-clay/10 text-clay">
                  Rupture de stock
                </span>
              </div>

              <p v-if="product.description" class="text-sm text-ink/70 leading-relaxed mb-4">
                {{ product.description }}
              </p>
              ...
              <template v-if="!isAdmin">
                <div class="mt-auto flex items-center gap-3">
                  <div class="flex items-center border border-mist rounded-pill">
                    ...
                    <button @click="product.stock !== undefined && quantity < product.stock && quantity++" class="w-10 h-10 flex items-center justify-center">+</button>
                  </div>
              ...
              <!-- Vue admin : juste l'état du stock -->
...
const cardMaxWidth = ref<string | null>(null);

const handleImageLoad = (event: Event) => {
  const img = event.target as HTMLImageElement;
  const maxDisplayHeight = window.innerHeight * 0.5;
  const displayedWidth =
    img.naturalHeight > maxDisplayHeight
      ? (img.naturalWidth * maxDisplayHeight) / img.naturalHeight
      : img.naturalWidth;

  cardMaxWidth.value = `${Math.min(displayedWidth, window.innerWidth * 0.9, 720)}px`;
};
